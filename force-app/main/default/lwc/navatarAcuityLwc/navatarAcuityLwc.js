...
handleFundraisingContactCreate(){
    if(this.fdrConPermissions['isCreateable']){
        this.publishEvent((this.objectApiName === 'Contact' ? 'Contact' : 'navpeII_Dev14__Fundraising__c') + '.New_Fundraising_Contact');
    } else {
            ...
            .then((result) => {
                ...
                if(['Account', 'Contact'].includes(this.objectApiName)){
                    ...
                    if(result){
                        ...
                        // this.enableSpinner = false; // Bug 00048257 Fixed by Sudhanshu on 11-06-2025
                    }
                } else if(this.objectApiName === this.namespacePrefix + 'Fundraising__c'){
                    // this.enableSpinner = false;  // Bug 00048257 Fixed by Sudhanshu on 11-06-2025
                    this.conCols = cols;
...
processInteractionSecInfo(){
    ...
    if(this.completeActIdList.length > 0 || this.recordId == undefined){
        if(!this.isMobile)      // Bug 00045453,00045633 fixed by Sudhanshu 06-06-2024
        {
            handleInteractionsInfo({namespacePrefix : this.namespacePrefix, actIdList : this.completeActIdList, objApiName : this.objectApiName})
            .then((result) => {
                if(result && result.length > 0){
                 ...
                 // Bug 00048257 fixed by Sudhanshu on 12-06-2025
                else if([this.namespacePrefix + 'Fund__c', this.namespacePrefix + 'Fundraising__c',this.namespacePrefix + 'Pipeline__c'].includes(this.objectApiName)){
                    this.enableSpinner = false;
                }
                if(['Account', 'Contact'].includes(this.objectApiName)){
        ...
        // Bug 00048257 fixed by Sudhanshu on 12-06-2025
        if([this.namespacePrefix + 'Fund__c', this.namespacePrefix + 'Fundraising__c',this.namespacePrefix + 'Pipeline__c'].includes(this.objectApiName)){
            this.enableSpinner = false;
        }
    }
...
fetchActInfo(event){
    if(['timesRef', 'meetCallRef'].includes(event.detail.action.name)){
        ...
        this[NavigationMixin.GenerateUrl]({
            ...
            attributes: {
                apiName: "navpeII_Dev14__Interactions",
            },
            state: {
                c__header: header,
    ...
    //Modified by Anshika Ahuja W.R.TO Redirection
        ...
        let compDef = {
            componentDef: "navpeII_Dev14:navatarAcuityRedirectionLwc",
            attributes: {
                header: header,
...
renderedCallback() {
    ...
    highlight_back_td.innerText = `.refCellHighlightCls a{background: #d8d8d8;
        border-radius: 3px;}
        /*00047601; fixed by raju 14-04-2025*/
            ...
            if(result){
                ...
                if(result.conSecDynFieldHeader3){
                    this.conCols.splice(4, 0, { label: result.conSecDynFieldHeader3, fieldName: 'roleRef3', type: 'picklist', hideDefaultActions: true, editable: this.dealTeamPermissions['isEditable'], typeAttributes: {
                        ...
                        fieldApiName: 'navpeII_Dev14__Team_Member_Role__c',
    ...
    if(idsToDelete.length == 0) {
        ...
        .then(result => {
            ...
            this[NavigationMixin.Navigate]({
                ...
                attributes: {
                    recordId: this.recordId,
                    objectApiName: 'navpeII_Dev14__Pipeline__c',
                    actionName: 'view'
                }
            });
        }).catch(error => {
...
handleNewDealTeamMember(){
    if(this.dealTeamPermissions['isCreateable']){
        if(this.dealTeamTabValue === 'Internal'){
            this.publishEvent('navpeII_Dev14__Pipeline__c.New_Team_Member');
        } else {
            this.publishEvent('navpeII_Dev14__Pipeline__c.New_Deal_Contact');
        }
    } else {
        //Msg updated by LK on 2024-05-13 to fix 00045572
        //Removed "Error!" by LK on 2024-07-04 to fix 00046365 - Code Reverted
...
createFundraisingUiInFund(){
    ...
    .then((result) => {
        ...
        // Bug 00048257 Fixed by Sudhanshu on 11-06-2025
        if(this.isFundPageLoad)
        {
            this.isFundPageLoad = false;
        }
        else{
            this.enableSpinner = false; 
        }
    ...
    if(this.objectApiName === (this.namespacePrefix + 'Fund__c') && this.fdrPermissions['isEditable']){
        ...
        copyData.forEach(item => {
            if (updateItem.hasOwnProperty(item.fieldName)) {
                updateItem[item.fieldApiName] = (item.fieldApiName == 'navpeII_Dev14__Investment_Likely_Amount_USD_mn__c') ? updateItem[item.fieldName].substring(updateItem[item.fieldName].indexOf(updateItem[item.fieldName].match(/\d+/)[0])) : updateItem[item.fieldName];//Bug45961 Fix By Harshwardhan
